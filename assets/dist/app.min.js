...
/* harmony import */ var _modules_main_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/main.js */ "./assets/src/js/modules/main.js");
...
$(document).ready(function () {
  _modules_echo_js__WEBPACK_IMPORTED_MODULE_0___default()('App ready');
});
...
var spinnerButton = document.querySelectorAll('.spinner-line');
var spinerMenu = document.querySelector('.js-menu');

var spinnerClassToggle = function spinnerClassToggle() {
  spinerMenu.classList.toggle('open');
  spinnerButton.forEach(function (elem) {
...
$(document).ready(function () {
  var iscrollPage = document.getElementById('iscroll');

  if (iscrollPage) {
    var myScroll = new iscroll__WEBPACK_IMPORTED_MODULE_0___default.a('#iscroll', {
      scrollbars: 'custom',
    ...
    var scrollStats = false;
    var oversizeSection = document.querySelector('.oversize');
    ...
    myScroll.on('scrollStart', function (e) {
      ...
      function oversizeRefScroll() {
        if ($('.oversize').scrollTop() === 0) {
          oversizeSection.removeEventListener('scroll', oversizeRefScroll); // myScroll.enable();
      ...
      if (!oversizeSection) {
        ...
        if (myScroll.currentPage.pageY + 1 !== myScroll.options.snap.length) {
          // we are NOT on oversize page
          myScroll.disable();
          scrollStats = false;
          window.setTimeout(function () {
            myScroll.enable();
    ...
    myScroll.on('scrollEnd', function (e) {
      ...
      var scrollbarIndicator = document.querySelector('.iScrollIndicator');
    ...
    $(document).on('touchend', function (e) {
      var touchScroll = e.originalEvent.changedTouches[0].clientY;
...
var simpleBarWrapper = document.querySelector('.simplebar-content-wrapper'); // lazy load & scroll fade in fade out

if (document.body.classList.contains('page-template-case-studies-cards')) {
  var allVerticalCards = document.querySelectorAll('.card-vertical-lazy');
  allVerticalCards[0].classList.add('card-first');
  allVerticalCards[allVerticalCards.length - 1].classList.add('card-last');
  var carsLazyLoad = document.querySelectorAll('.card-vertical-lazy');
  var overlay = document.querySelector('.sticky-container');
  var seeMoreBtn = document.querySelector('.sticky-container-link');
  var firstCardElement = document.querySelector('.card-first');
  var lazyContainer = document.querySelector('.lazy-load-all');
  var containerGap = document.querySelector('.lazy-flexible');
  var backTotop = document.querySelector('.back-to-top-cont');
  var backTotopHeight = Number(getComputedStyle(backTotop).height.replace('px', ''));
  var lozyObserver = new IntersectionObserver(function (entries, observer) {
  ...
  carsLazyLoad.forEach(function (card) {
    lozyObserver.observe(card);
  }); // function

  var scrollToSecondRow = function scrollToSecondRow() {
    var getContainerPadding = getComputedStyle(lazyContainer).paddingTop;
    var containerPadding = Number(getContainerPadding.replace('px', '')); // take gap size from css style

    var gap = getComputedStyle(containerGap).gridGap.slice(0, 2); // take gap to number

    var gridGapNumb = Number(gap);
    var scrollHeight = firstCardElement.clientHeight * 2 + (containerPadding + gridGapNumb + backTotopHeight); // scroll point
  ...
  simpleBarWrapper.addEventListener('scroll', function (event) {
    var firstTrigger = document.querySelector('.card-first').getBoundingClientRect().height;
...
var clientLogo = document.querySelectorAll('.client-logo--item');

var clientLogoRef = function clientLogoRef() {
  var width = document.body.clientWidth;
  var height = document.body.clientHeight;
  clientLogo.forEach(function (logo) {
    // console.log(logo);
  ...
  if (width <= 415 && height <= 844) {
    ...
    for (var _i = 0; _i < clientLogo.length; _i++) {
      if (_i >= 10) {
        clientLogo[_i].classList.add('disabled');
      }
    }
  }

  if (width <= 768 && height <= 1024) {
    for (var _i2 = 0; _i2 < clientLogo.length; _i2++) {
      if (_i2 >= 21) {
        clientLogo[_i2].style.display = 'none';
      }
    }
  }
};
...
var BrowserDetect = {
  init: function init() {
    ...
    this.OS = this.searchString(this.dataOS) || "an unknown OS"; // alert(`broeser ${this.browser}, version ${this.version}, os ${this.OS}`);
